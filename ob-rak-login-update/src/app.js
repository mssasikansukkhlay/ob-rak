...
function authEmailForUserId(value) {
  const normalized = normalizeUserId(value);
  return `uid.${normalized}@login.obrak.app`;
}
...
function fromSupabaseError(error, fallback = 'เกิดข้อผิดพลาด กรุณาลองใหม่') {
  ...
  if (raw.toLowerCase().includes('email not confirmed')) message = 'Supabase ยังเปิด Confirm email อยู่ กรุณาปิดการยืนยันอีเมลสำหรับระบบ USER ID นี้';
  if (raw.toLowerCase().includes('user already registered')) message = 'USER ID นี้ถูกใช้แล้ว กรุณาเลือก USER ID อื่น';
...
async function api(path, options = {}) {
  ...
  try {
    if (method === 'POST' && pathname === '/api/auth/register') {
      ...
      if (!userId || !username || password.length < 8 || !body.consent) {
      ...
      const email = authEmailForUserId(userId);
      ...
      return {
        message: 'สร้างบัญชีแล้ว แต่ Supabase เปิด Confirm email อยู่ ระบบ USER ID ต้องปิด Confirm email ก่อนจึงจะเข้าสู่ระบบได้'
      };
    }
    ...
    if (method === 'POST' && pathname === '/api/auth/login') {
      ...
      const { data, error } = await supabase.auth.signInWithPassword({
        email: authEmailForUserId(userId),
...
function bindEvents() {
  ...
  elements.forgotPasswordButton?.addEventListener('click', () => {
    setMessage(elements.loginMessage, 'ระบบ USER ID เวอร์ชันต้นแบบยังไม่รองรับรีเซ็ตรหัสผ่านทางอีเมล กรุณาติดต่อผู้ดูแลระบบ');
  });
